...
### 1.1 L 形路径左移阶段车身后退 / wobble

**症状**：body 帧模式下，先直行后侧移，侧移阶段车身向后退 6~16cm，且左右扭动，轨迹不直。

**根因**：差速轮底盘被当作全向轮发了 `tw.linear.y`（侧移指令）。MCU 收到无法执行的指令后自行混算，产生寄生速度。

- 泄漏公式：`tw.linear.x = sin(yaw) × vy_map ≈ sin(68°) × 0.2 = 0.185 m/s`
- 这个寄生速度在 P 控制器还没开始修正时就已经存在

**解法**：废弃 `tw.linear.y`，侧移改为 **旋转 90° 朝目标 → 直走 → 旋转回来**。只用差速轮原生自由度 `tw.linear.x` + `tw.angular.z`。

### 1.2 调参无法收敛

**症状**：K_LIN 从 1.0 → 2.5 有改善（后退从 15cm 降到 ~7cm），但继续加增益（→4.0）无效，甚至更差。

**根因**：P 控制器输出被 `V_LIN=0.2` 硬钳制，K_LIN 增大到一定程度后 vx_map 触碰 ±0.2 上限，再加增益也不会产生更多修正力。

**解法**：同上，这是结构问题，不是参数问题。

### 1.3 Y 方向恒定超调 +6~11cm

**症状**：所有实验中左移距离都比目标多 6~11cm，无论 K_LIN 多大。

**根因**：MCU 对 `tw.linear.y` 的混算结果无法被 P 控制器干预。差速轮的 Y 轴滑动来自物理约束，不是软件可控量。

**解法**：同上，取消 tw.linear.y。
...
- `小车调节日志.txt` — 完整原始实验数据（Run 4~10）
- `调参极限分析报告.md` — 数学推导与实验分析
- `真机信息.md` — 硬件清单与当前状态
