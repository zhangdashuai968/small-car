...
void ImuFilterRos::publishFilteredMsg(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  //imu_msg->angular_velocity.x = imu_msg_raw->angular_velocity.x;
  //imu_msg->angular_velocity.y = imu_msg_raw->angular_velocity.y;
  //imu_msg->angular_velocity.z = imu_msg_raw->angular_velocity.z * angular_scale_;
  if(imu_msg->angular_velocity.x > 0.02 || imu_msg->angular_velocity.x < -0.02) 
      imu_msg->angular_velocity.x = imu_msg_raw->angular_velocity.x;
  else
      imu_msg->angular_velocity.x = 0.0;

  if(imu_msg->angular_velocity.y > 0.02 || imu_msg->angular_velocity.y < -0.02) 
      imu_msg->angular_velocity.y = imu_msg_raw->angular_velocity.y;
  else
      imu_msg->angular_velocity.y = 0.0;
  
  if(imu_msg->angular_velocity.z > 0.02 || imu_msg->angular_velocity.z < -0.02) 
      imu_msg->angular_velocity.z = imu_msg_raw->angular_velocity.z * angular_scale_;
  else
