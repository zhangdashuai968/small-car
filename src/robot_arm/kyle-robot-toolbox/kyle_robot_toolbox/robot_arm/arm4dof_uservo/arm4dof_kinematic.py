...
class Arm4DoFKinematic:
    ...
    JOINT_ANGLE_DEFAULT = np.radians([0, -90.0, 90.0, 0])
    # 关节角度下限, 单位: 弧度
    JOINT_ANGLE_LOWERB =  np.radians(np.float32([-135.0, -180.0, -135.0, -135.0]))
    # 关节角度上限制, 单位: 弧度
    JOINT_ANGLE_UPPERB = np.radians(np.float32([135.0, 135.0, 135.0, 135.0]))
    ...
    def inverse_kinematics(self, x, y, z, alpha=180):
        ...
        if valid:
            ...
            candi_joint_angle_list.append([90-deg1, -deg2, 90-deg3, 90-deg4])
...
if __name__ == '__main__':
    ...
    y = 0
    z = 0
    arm = Arm4DoFKinematic()
    angle_list = arm.inverse_kinematics(x, y, z, alpha=180)
    print(angle_list)
    if len(angle_list) != 0:
        tx, ty, tz, pitch = arm.forward_kinematics(angle_list[0][0], angle_list[0][1], angle_list[0][2], angle_list[0][3])
        print(pose)
        if abs(tx-x) > 5 or abs(ty-y) > 5 or abs(tz-z) > 5:
            print('err')
        else:
            print('ok')
