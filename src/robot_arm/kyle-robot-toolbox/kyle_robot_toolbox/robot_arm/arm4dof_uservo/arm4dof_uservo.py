...
# 阿凯机器人工具箱
# - 导入执行器 串口舵机管理器
from kyle_robot_toolbox.actuator.uservo import UartServoManager 
# - 导入五自由度机械臂运动学求解器
from kyle_robot_toolbox.robot_arm.arm4dof_uservo import Arm4DoFKinematic
# - 导入轨迹规划算法
from kyle_robot_toolbox.trajectory_plan import minimum_jerk_plan, minimum_jerk_seq
# - 高精度时钟模块
...
class Arm4DoFUServo:
	...
	# 旋转关节名称列表
	joint_name_list = ["joint1", "joint2", "joint3", "joint4", "left_gripper_joint"]
	# 等待超时阈值
	wait_timeout = 10
	# 轨迹执行回调函数执行周期, 几个时间块执行一次
	trajectory_callback_period = 20
	
	def __init__(self, is_init_pose:bool=True, \
     		config_folder="config", is_debug=False):
		'''机械臂初始化'''
		self.config_folder = config_folder
		# 载入配置文件
		arm_config_path = os.path.join(self.config_folder, 'arm.yaml')
		with open(arm_config_path, 'r', encoding='utf-8') as f:
			self.config = yaml.load(f.read(), Loader=yaml.SafeLoader)
		
		# 提取端口号
		self.device = self.config["device"]
		# 关节角度与电机角度的映射
		self.joint2servo_k = np.float32(self.config['joint2servo']['k'])
		self.joint2servo_b = np.float32(self.config['joint2servo']['b'])
		self.servo2joint_k = 1.0 / self.joint2servo_k
		self.servo2joint_b = - self.joint2servo_b / self.joint2servo_k
		# 提取俯仰角自动生成的参数
		self.pitch_panel_a = self.config["pitch_panel"]["a"]
		self.pitch_panel_b = self.config["pitch_panel"]["b"]
		self.pitch_panel_c = self.config["pitch_panel"]["c"]
		# 提取姿态列表
		self.pose_dict = self.config["pose_dict"]
		
		# 创建串口对象
		try:
			self.is_debug = is_debug
			
			self.uart = serial.Serial(port=self.device, baudrate=115200,\
					 parity=serial.PARITY_NONE, stopbits=1,\
					 bytesize=8,timeout=0)
			# 创建串口舵机管理器
			self.uservo = UartServoManager(self.uart, \
       			srv_num=self.SERVO_NUM, is_scan_servo=True, is_debug=False)
			# 创建运动学求解器
			self.kinematic = Arm4DoFKinematic(is_debug=is_debug)
			...
			self.theta_upperb.append(np.pi/2)
   			# Z轴误差补偿系数
			file_path = os.path.join(config_folder, "handeye_calibration", "z_error_polyfit.txt")
			...
			logging.error('或者检查下当前的串口是不是被别的脚本占用了')
			# 退出
			exit(-1)
		
	def joint_name2servo_id(self, joint_name):
		'''将关节名称转换为舵机ID'''
		return self.JOINT_SERVO_ID_DICT[joint_name]
	...
	def set_servo_velocity(self, speed:float):
		'''设置舵机的转速 单位°/s'''
		self.uservo.mean_dps = max(min(abs(speed), self.SERVO_SPEED_MAX), self.SERVO_SPEED_MIN)
	...
	def set_joint_angle(self, joint_name, theta, interval=None, \
		...
		# 关节角度转换为驱动器舵机原始角度
		# 根据关节的弧度计算出舵机的原始角度
		angle = self.joint2servo_k[servo_id]*theta + self.joint2servo_b[servo_id] 
	...
	def trajectory_plan(self, joint_name, theta_e:float, T:float, w_s:float=0.0, w_e:float=0.0, a_s:float=0.0, a_e:float=0):
		...
		t_arr, theta_arr = minimum_jerk_seq(T, c, delta_t=self.TRAJECTORY_DELTA_T)
	...
	def set_joint_angle_soft(self, joint_name, joint_angle, T=1.0, max_power=0, callback=None):
		...
		while True:
			...
			m = int(np.ceil((t_end - t_start) / self.TRAJECTORY_DELTA_T))
			if m == 0:
				m = 1
			i += m
			# 补齐所需延迟等待的时间
			sleep_s(m*self.TRAJECTORY_DELTA_T - (t_end - t_start))
			# 回调函数执行
	...
	def get_servo_angle(self, joint_name):
		'''读取舵机原始角度'''
		servo_id = self.JOINT_SERVO_ID_DICT[joint_name]
	...
	def get_joint_angle(self, joint_name):
		...
		# 将舵机角度转换为关节弧度
		joint_angle = self.servo2joint_k[servo_id] * servo_angle + self.servo2joint_b[servo_id]
  		# 获取关节状态
		return joint_angle
	...
	def get_tool_pose(self, return_type="XYZ_Pitch_Roll"):
		'''获取工具坐标系的位姿'''
		# 获取当前的关节角度列表
		cur_joint_angle =  np.degrees(self.get_joint_angle_list()[:4])
                #print(cur_joint_angle)
		# 返回的数据格式: [x6, y6, z6, pitch, roll]
		return self.kinematic.forward_kinematics(cur_joint_angle[0], cur_joint_angle[1], cur_joint_angle[2], cur_joint_angle[3])
	
	def inverse_kinematic(self, tool_posi, is_debug=False):
		'''逆向运动学
		@tool_posi: 工具末端的位置
		@tool_pitch: 工具末端的俯仰角
		'''
		# 获取候选关节角度列表
		tx, ty, tz = tool_posi
		candi_joint_angle_list = self.kinematic.inverse_kinematics(tx, ty, tz)
		# 选取一个
		if len(candi_joint_angle_list) == 0:
			return False, None
		else:
			# TODO 选取一个最合适的解
			return True,  np.radians(candi_joint_angle_list[0])
	...
	def auto_gen_pitch(self, p_tool:list):
		...
		pitch = self.pitch_panel_a*x0 + self.pitch_panel_b*z + self.pitch_panel_c
		if self.is_debug:
			print(f"自动生成俯仰角: {pitch}")
		return pitch
	
	def set_tool_pose(self, tool_posi=None, tool_pitch=None, \
		pose_name=None, is_soft_move=True, T=1.0, \
		...
		if pose_name is not None:
			...
			if pose_name in self.pose_dict.keys():
				# 角度转换为弧度
				thetas = np.radians(self.pose_dict[pose_name])
			else:
				# 未知的位姿名称
				return False
		else:
			# 逆向运动学
			ret, thetas = self.inverse_kinematic(tool_posi, is_debug=is_debug)
			# 判断是否有解
			if not ret:
				logging.warn('机械臂末端不能到达: {} 俯仰角={}'.format(\
					tool_posi, tool_pitch))
				return False
		# 控制关节
	...
	def set_damping(self, power=0):
		'''设置为阻尼模式'''
		for servo_id in range(self.SERVO_NUM):
