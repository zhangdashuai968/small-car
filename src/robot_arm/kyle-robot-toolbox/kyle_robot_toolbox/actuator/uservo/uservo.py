...
import struct
# 阿凯机器人工具箱
from kyle_robot_toolbox.system import get_cur_time_s, sleep_s
# 设置日志等级
class Packet:
	'''数据包'''
...
class UartServoInfo:
	...
	def is_stop(self):
		...
		if self.last_angle_error is None:
			...
			self.last_sample_time = get_cur_time_s()
		...
		if abs(self.last_angle_error - angle_error) > 0.2:
			self.last_angle_error = angle_error
			self.last_sample_time = get_cur_time_s() # 更新采样时间

		if (get_cur_time_s() - self.last_sample_time) > 1:
			# 已经有1s没有更新误差了, 舵机卡住了
