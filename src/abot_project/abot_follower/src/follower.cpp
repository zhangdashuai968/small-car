...
private:
  ...
  std::string   camera_model_; /**< Enable/disable following; just prevents motor commands */


public:
  RikibotFollower() : min_y_(0.1), max_y_(0.5),
  ...
  { 
    ...
    sub_= it.subscribe("depth_image", 1, &RikibotFollower::imagecb, this);
  ...
  void imagecb(const sensor_msgs::ImageConstPtr& depth_msg)
  {
    ...
    for (int v = 0; v < (int)depth_msg->height; ++v){
     for (int u = 0; u < (int)depth_msg->width; ++u){
       //if(strcmp(camera_model_, "astra"))
       if(!camera_model_.compare("astra"))
         depth = depth_image_proc::DepthTraits<float>::toMeters(depthimage.at<float>(v, u));
       if(!camera_model_.compare("astrapro"))
       //if(strcmp(camera_model_, "astrapro"))
         depth = depth_image_proc::DepthTraits<float>::toMeters(depthimage.at<float>(v, u));
       if(!camera_model_.compare("intel"))
       //if(strcmp(camera_model_, "intel"))
...
{
    ros::init(argc, argv, "rikibot_follower_node");
    RikibotFollower rikibot_follower;
    ros::spin();
    return 0;
}
