...
int main(int argc, char** argv){
  ros::init(argc, argv, "rikibot_multi");
  ...
  private_nh.param<std::string>("pub_vel", pub_vel, "cmd_vel");  
  ...
  private_nh.param<std::string>("target_frame", target_frame, "riki2");

  ros::Publisher slave_vel = node.advertise<geometry_msgs::Twist>(pub_vel, 10); //发布原始数据

 // 使用tf_prefix参数将frame_name解析为frame_id
  tf_prefix_ = tf::getPrefixParam(private_nh); 
  base_frame = tf::resolve(tf_prefix_, base_frame);
  target_frame = tf::resolve(tf_prefix_, target_frame);
  ...
  while (node.ok()){
    ...
    try{  
        ...
        listener.lookupTransform(base_frame, target_frame, ros::Time(0), transformSM);
    }
    ...
    angular_z = atan2((transformSM.getOrigin().y()), (transformSM.getOrigin().x()));
    linear_x = sqrt(pow((transformSM.getOrigin().x()), 2) + pow((transformSM.getOrigin().y()), 2));
