...
class AbotSpeakerDriver(object):
    VENDOR_ID = 0x2886
    PRODUCT_ID = 0x0018
    TIMEOUT = 100000
    ...
    def __init__(self):
        ...
        if not self.dev:
            raise RuntimeError("Failed to find Abot Speaker device")
        rospy.loginfo("Initializing Abot Speaker Device")
        self.dev.reset()
        ...
        self.set_led_trace()
        rospy.loginfo("Abot Speaker device initialized (Version: %s)" % self.version)
...
class AbotSpeakerAudio(object):
    def __init__(self, on_audio, channels=None, suppress_error=True):
        ...
        if self.device_index is None:
            rospy.logwarn("Failed to find speaker device by name. Using default input")
