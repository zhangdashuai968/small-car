...
class AbotFaceDetect():
    def __init__(self):
        rospy.init_node('abot_facename_detect_node', log_level=rospy.INFO)
	...
	dir_path = dir_path.replace('abot_object_detect/node', 'abot_object_detect/')
        dir_path += 'image/'


        id1_image = face_recognition.load_image_file(dir_path + "kevin.jpg")
	id1_face_encoding = face_recognition.face_encodings(id1_image)[0]

	# Load a second sample picture and learn how to recognize it.
	id2_image = face_recognition.load_image_file(dir_path + "zhuohua.jpg")
	...
	process_this_frame = True
        ...
        self.pub_name = rospy.Publisher("/face_name", String, queue_size=1)
		...
		# Display the results
        	...
        	# Scale back up face locations since the frame we detected in was scaled to 1/4 size
                ...
                if known_face_names[0] in face_names :
                    rospy.loginfo("have face, detect name is: %s",  name)
                    face_name = String()
                    face_name.data = name
                    self.pub_name.publish(face_name)
