...
class VLM:
    ...
    def yi_vision_api(self, prompt='检测图中红色方块'):
        ...
        try:
            result = self.extract_json_result(content)
            # 如果返回的是列表，取第一个元素
            if isinstance(result, list):
                if not result:
                    rospy.logwarn("大模型未检测到目标（返回空列表）")
                    return None
                result = result[0]
    ...
    def extract_json_result(self, content):
        """从任意格式文本中提取JSON对象或数组，容忍markdown/多余字符"""
    ...
    def normalize_bbox(self, bbox_2d, img_w, img_h):
        """归一化bbox到图像像素坐标：VLM可能返回0-1000归一化坐标或真实像素坐标"""
        x_min, y_min, x_max, y_max = map(float, bbox_2d)
        if max(x_min, x_max) > img_w or max(y_min, y_max) > img_h:
            rospy.logwarn(
                f"bbox坐标超出图像尺寸，按0-1000坐标归一化: bbox={bbox_2d}, image=({img_w},{img_h})"
            )
    ...
    def post_processing_viz(self, result, img_path):
        ...
        bbox_2d = self.normalize_bbox(bbox_2d, img_w, img_h)
