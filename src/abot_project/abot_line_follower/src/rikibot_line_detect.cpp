#include "rikibot_line_detect.hpp"
...
void RikibotLineDetect::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
...
cv::Mat RikibotLineDetect::Gauss(cv::Mat input) {
    cv::Mat output;
...
void RikibotLineDetect::SetLineHsvScalar(cv::Scalar lower, cv::Scalar upper)
{
    LowerColor = lower;
    UpperColor = upper;
...
int RikibotLineDetect::colorthresh(cv::Mat input) {
    ...
    cv::cvtColor(input, RikibotLineDetect::img_hsv, CV_BGR2HSV);
    cv::inRange(RikibotLineDetect::img_hsv, LowerColor,
            UpperColor, RikibotLineDetect::img_mask);
    img_mask(cv::Rect(0, 0, w, 0.8*h)) = 0;
    // Find contours for better visualization
    cv::findContours(RikibotLineDetect::img_mask, v, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);
    ...
    cv::Moments M = cv::moments(RikibotLineDetect::img_mask);
    if (M.m00 > 0) {
        cv::Point p1(M.m10/M.m00, M.m01/M.m00);
        cv::circle(RikibotLineDetect::img_mask, p1, 5, cv::Scalar(155, 200, 0), -1);
    }
    c_x = M.m10/M.m00;
    ...
    cv::namedWindow("Rikibot View");
    imshow("Rikibot View", input);
    return (w/2-c_x);
}
