    ...
    heading_(0)
{
    ...
    odom_publisher_ = nh_.advertise<nav_msgs::Odometry>("raw_odom", 100);
    velocity_subscriber_ = nh_.subscribe("raw_vel", 100, &RikiBase::velCallback, this);
    battery_sub_ = nh_.subscribe("battery", 0.2, &RikiBase::batteryCallback, this);
    nh_private.param<float>("linear_scale", linear_scale_, 1.0);
    nh_private.param<float>("low_battery", low_battery_, 11.6);
    nh_private.param<std::string>("frame_id", frame_id_, "odom");
    nh_private.param<std::string>("child_frame_id", child_frame_id_, "base_footprint");

    
    //ROS_INFO("linear_scale_: %f", linear_scale_);
}

void RikiBase::batteryCallback(const riki_msgs::Battery& bat)
{
	float cur_battery = bat.battery;
	if (cur_battery < low_battery_){
		ROS_WARN("Current Voltage: %f ,Low Battery, Low Battery  Please Recharge!!", cur_battery);
	}
}
