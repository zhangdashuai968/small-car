...
- **已切换回 move_base + TEB 局部规划器**，不再绕开。此前自闭环原语方案（`auto_task_runner.py` 直接发 `cmd_vel`）已废弃。
- TEB 配置使用全向底盘参数（`teb_omni_planner_params.yaml`），正在针对麦克纳姆轮调优。
- 定位用 `localize.launch`（amcl + move_base），初值 (0,0,0)；建图与 amcl 互斥（都发 `map→odom`，永远二选一）。
- **当前阶段：TEB 参数调优进行中**——速度/加速度上限已放开、运动学约束已解除、同伦类规划已启用，细节见最新 commit `dc64d56`。
