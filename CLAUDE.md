...
| ✅ | 真机随手记录（`logs/小车调节日志.txt`） | 速查表（cheatsheets） |
...
- 调试过程中的关键操作、命令输出、异常信息，**实时追加**到 `logs/小车调节日志.txt`
- 格式：`[HH:MM] 内容描述`
...
- **密码**：`scripts/ssh-car.py` 中已有硬编码密码，新代码**绝不**再硬编码。用环境变量或配置文件读取
- **参数修改前先备份**：改 launch/param 文件前，记录原始值
- **运动控制前确认急停**：给出 `/cmd_vel` 或 move_base 命令前，提醒队友确认急停按钮可达
- **机械臂回 home 再断电**：每次会话结束前提醒：先归位再 Ctrl+C
- **底盘差速轮限制**：`twist.linear.y` 对差速轮底盘无效（详见 `reports/调参极限分析报告.md`），使用"旋转→直行→旋转"替代侧移
...
- 硬件参考：`README.md`（硬件清单和已知 bug）
- 调参分析：`reports/调参极限分析报告.md`（差速轮轨迹漂移根因分析）
- 踩坑手册：`TROUBLESHOOTING.md`
