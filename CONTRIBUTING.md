...
合并前用 Claude Code 的 `/review` 过一遍代码，**尤其是会在挡板边上跑的运动/导航脚本**（一个 bug = 撞墙 = 判负）。

- **怎么用**：代码改动走 feature 分支 → 推上去开 PR → 在 Claude Code 输入框打 `/review <PR号>`，问题会作为行内评论贴到 PR 上。
- **力度**：运动脚本用 `high` 或 `max`（宁可多报不确定的，也别漏）。
- **注意**：直推 `main` 时 `/review` 没 diff 抓手——所以**代码尽量走 PR**，别直推（文档/配置直推无妨）。
- ⚠️ `/review` 只查代码对不对，查不了车上跑得对不对——上车前静态预检，**不替代真机单步验证**。
...
- **写完日志/报告，结尾必须写清「下一步该干什么」**（交接用，别让接手的人靠猜）：还没闭环的问题、下次先做哪一步、有什么坑要避开。报告模板的「下一次会话建议」「未解决的问题」就是干这个的——别留空。
